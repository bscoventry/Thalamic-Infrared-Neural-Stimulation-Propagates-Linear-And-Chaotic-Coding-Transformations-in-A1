...
% pR = estimatePMF(R);
% pRS = estimatePMF(RS);
[pR,~] = histcounts(R,762,'Normalization', 'probability');%estimatePMF(RSamp);
[pRS,~] = histcounts(RS,762,'Normalization', 'probability');%(RSSamp);
[~,numBinsR] = size(pR);
[~,numBinsRS] = size(pRS);
...
for ck=1:length(useTrials)
    ...
    [pRSamp,~] = histcounts(RSamp,762,'Normalization', 'probability');%estimatePMF(RSamp);
    [pRSSamp,~] = histcounts(RSSamp,762,'Normalization', 'probability');%(RSSamp);
