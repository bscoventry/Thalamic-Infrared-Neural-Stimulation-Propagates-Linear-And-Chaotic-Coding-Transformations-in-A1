...
df = pd.DataFrame(columns=['DataID', 'Electrode', 'EnergyPerPulse','ISI','NPulses','MI'])
    ...
    try:
        ...
        for electrode in sortedLFPs.keys():
            ...
            for bc,energies in enumerate(curDataElec.keys()):
                ...
                Iqe = Spike_Processed.estimateEntropy_Continuous(R,Rs,doQE=True)
                Iarray[bc] = Iqe
                
                df.loc[-1] = [word,str(electrode),str(energies),ISI,NPul,Iqe]
