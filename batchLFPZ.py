...
df = pd.DataFrame(columns=['DataID', 'Electrode', 'EnergyPerPulse','ISI','NPulses','ZMax','meanLFP','sdderLFP'])
...
for ck, word in enumerate(dataPath):
    ...
    try:
        ...
        arrayLFPsSD = SpikeClass.sortMeanByElectrode16(sderLFPs)
        nEnergies = arrayLFPs.keys()
        for ck in arrayLFPs.keys():
                    [ny,nx,nt] = np.shape(arrayLFPs[ck])
                    for bc in ny:
                         for jk in nx:
