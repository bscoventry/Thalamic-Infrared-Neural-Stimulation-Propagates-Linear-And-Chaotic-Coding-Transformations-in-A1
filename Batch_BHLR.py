...
maxSTA = np.nanmax(df.STAMeanAUC.values)
...
def BHLR(dependentVar_og,indVar1_og,indVar2_og,animNames,electrodes, hyperpriorSigma, islogDep=1,islogIndep=1, model = 'StudentT', saveTrace=1, fileName='TestName', rootSavePath = 'TestName'):
    ...
    if __name__ == '__main__':      #Need to do this for windows. No idea why. Something to do with hyperthreading
        ...
        #plt.savefig('/INSxAuditory/' + 'Figures/' + fileName + '_' + figTitle + '.jpg', format='jpg')
        ...
        numBurnIn = 500     #Burning in samples help increase convergence in mcmc of the samples we are keeping
        numSamples = 2000
...
""" 
all_z = ['STAMeanAUC'] #[var for var in collected_var if 'Diff' in var]  # Dependent variable set
