...
if __name__ == '__main__':       #This statement is to allow for parallel sampling in windows. Linux distributions don't require this. Doesn't hurt Linux to have this either.
    ...
    # AMDepthData = data3
    lepp = data.lepp                     #This is our modulation depth vector
    ...
    Sinks = Sinks*1000
    Sinks = Sinks.astype(aesara.config.floatX) #Make sure response variable is in a tensor like structure for computaiton. This is the only time we need to directly invoke aesara
    lepp = np.asarray(lepp)                     #Make sure Xs are not panda series, aesara is not a fan of those sometimes.
    Sources = Sources.astype(aesara.config.floatX)
    """
    ...
    # Priors for unknown model parameters
        α = pm.Normal('α', mu=prMean, sigma=5, shape=k, 
                  testval=np.array([Y.mean()]*2)) #Intercept
        β = pm.Normal('β', mu=prMean, sigma=5, shape=k)
        σ  = pm.HalfCauchy('σ', 5, shape=k)  #Noise
        #eps = pm.HalfCauchy("eps", 5,shape=k)
        mu = α + (lepp[:,None]*β)
        
    
        
        likelihood = pm.NormalMixture('likelihood', π, mu, sigma=σ, observed=np.log(Sinks+0.01))
        ...
        # reg = pm.Deterministic('reg', a + (B*lepp))     #Deterministic is for non probabilistic data. This is a modification to help sampling, inference is still probabilistic
        # likelihood =  pm.NormalMixture("x", w=weights, mu=reg, sigma=eps, observed=np.log(Sinks+0.01))#pm.StudentT('Y',mu = reg, sigma = eps,nu=nu, observed = np.log(Sinks+0.01))    
        """
    ...
    intercept = trace.posterior["α"]                #Grab the posterior distribution of a
    Slope = trace.posterior["β"]                    #Grab the posterior distribution of B
    ...
    az.plot_posterior(trace, var_names=['α', 'β','π'], point_estimate='mode',hdi_prob=0.95)
    plt.show()
    fig = plt.figure(figsize=(9, 6))
    ax = fig.add_subplot(
    111,
    xlabel=r"ln(Energy Per Pulse)",
    ylabel=r"Sink Magnitude",
    ...
    az.plot_trace(trace,var_names=['α', 'β', 'π'])
    plt.show()
    pdb.set_trace()